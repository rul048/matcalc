...
def test_energetics_calc(
    ...
    assert result["formation_energy_per_atom"] == pytest.approx(-1.8224377632141113, rel=1e-3)
    assert result["cohesive_energy_per_atom"] == pytest.approx(-4.050449167070313, rel=1e-3)
    ...
    for key in (
        ...
        assert key in result, f"{key=} not in result"
    assert result["formation_energy_per_atom"] == pytest.approx(-1.8427848695703126, rel=1e-3)
    assert result["cohesive_energy_per_atom"] == pytest.approx(-4.050449167070313, rel=1e-3)
